...
Function .onInit
    ...
    MessageBox MB_YESNOCANCEL|MB_ICONQUESTION \
        "检测到已安装 ${PRODUCT_NAME} 版本 $R0$\n$\n是否升级到新版本 ${PRODUCT_VERSION}？$\n$\n$\n是 - 升级（保留用户数据）$\n否 - 完全覆盖安装$\n取消 - 退出安装程序" \
        IDYES upgrade IDNO no_upgrade IDCANCEL done_check

    upgrade:
        ; 询问是否先卸载旧版本
        MessageBox MB_YESNO|MB_ICONQUESTION \
            "升级模式：建议先卸载旧版本再安装新版本$\n$\n是否现在自动卸载旧版本？" \
            IDYES do_uninstall IDNO do_upgrade
...
Section "Uninstall"
    ...
    ; 询问是否删除用户数据目录
    MessageBox MB_YESNO|MB_ICONQUESTION \
        "是否删除用户数据目录（包含用户设置和文件）？$\n$\n选择'否'将保留 phcode_data 目录" \
